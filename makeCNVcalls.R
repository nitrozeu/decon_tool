...
#write.table(ExomeCount,"exomecount.txt")

cnv.calls = NULL
refs<-list()
models<-list()

#print(ExomeCount$start)
#print(ExomeCount$end)
#print(ExomeCount$exon)
...
if(plotOutput!="None"){
  ...
  for(call_index in 1:nrow(cnv.calls_plot)){
    ...
    ## edited part
    Data2<-Data1[Data1$testref=="Test Sample",]
    if(nrow(Data2)>1){
      for(i in 1:(nrow(Data2)-1)){
        if(is.na(Data2$exonRange[i]+1)==FALSE & is.na(Data2$exonRange[i+1])==FALSE){
